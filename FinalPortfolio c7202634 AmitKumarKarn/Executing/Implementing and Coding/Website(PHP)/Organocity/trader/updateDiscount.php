      ...
      <?php include 'includes/topbar.inc.php'; ?>
        ...
        <?php 
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800 mx-auto">Update Discounts</h1>
          </div>
              ...
              <div class="card shadow mb-4">
                ...
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  ...
                  <?php
                    $pid = $_GET['pid'];
                    $sid = $_GET['sid'];
                    $scr = oci_parse($trader->connect(), "SELECT * FROM PRODUCT WHERE PRODUCT_ID = $pid AND SHOP_ID = $sid");
                    oci_execute($scr);
                  ...
                  <div class="dropdown no-arrow">
                    ...
                    <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
                      ...
                      <?php 
                            //$shop_id = $trader->getShopId();
                            $tid = $_SESSION['user_id'];
                            $sc = oci_parse($trader->connect(), "SELECT * FROM DISCOUNT d, PRODUCT p WHERE p.PRODUCT_ID = d.PRODUCT_ID AND d.USER_ID = $tid");
                            oci_execute($sc);
                            while($row = oci_fetch_assoc($sc))
                            {
                        ?><a class="dropdown-item" href="updateDiscount.php?pid=<?php echo $row['PRODUCT_ID']; ?>&sid=<?php echo $_GET['sid']; ?>"><?php echo $row['PRODUCT_NAME']; ?></a><?php
                            }
                      ?>
                    </div>
                  </div>
                ...
                <div class="card-body modify-form">
                    <?php  
                        $productid = $_GET['pid'];
                        $sid = $_GET['sid'];
                        $scrpt = oci_parse($trader->connect(), "SELECT * FROM DISCOUNT WHERE PRODUCT_ID = $productid");
                        oci_execute($scrpt); 
                        $acount = 0;
                    ...
                    <div class="row">
                         ...
                         <div class="col-lg-10 mb-4">
                            ...
                            <div class="input-group col-lg-12 mb-3">
                                ...
                                <input id="discountName" type="text" name="dname" value="<?php if(isset($_POST['dname'])){ echo $_POST['dname']; } else {  echo strtoupper($row['DISCOUNT_NAME']); }?>" class="form-control bg-white border-left-0 border-md">
                            </div>
                            ...
                            <div class="input-group col-lg-12 mb-3">
                                ...
                                <input id="discountPercent" type="text" name="dpercent" value="<?php if(isset($_POST['dpercent'])){ echo $_POST['dpercent']; } else {  echo $row['DISCOUNT_PERCENT']; } ?>" class="form-control bg-white border-left-0 border-md">
                            </div>
                        ...
                        <div class="form-group col-lg-12 mx-auto mb-0">
                            <input type="submit" name="updateDiscountSubmit" class="btn btn-primary btn-block py-2 font-weight-bold "  value="Update Discount">
                        </div>

                    </div>
                </form>
                        <?php } if($acount == 0) { echo '<span class="text-muted">Discount for this product is not added yet!<br />
                        Go to <a href="addDiscount.php?pid='.$_GET['pid'].'&sid='.$_GET['sid'].'"> Add Discounts </a> to add discount to this product.</span>'; } ?>
                </div>
              </div>
